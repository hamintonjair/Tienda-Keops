...
foreach ($_SESSION['arrCarrito'] as $producto) {
    $subtotal += $producto['USD'] * $producto['cantidad'];
 
  
}
$total += $subtotal + $costo_evio;

$titulo  = $this->Helpers->getInfoPage(PTERMINOS);

for ($p = 0; $p < count($titulo); $p++) { 
	
	$tituloTerminos = $titulo[$p]->titulo;	
	$infoTerminos  = $titulo[$p]->contenido;			
...
<script 
      src="https://www.paypal.com/sdk/js?client-id=<?=$idcliente_paypal ?>&currency=<?= CURRENCY ?>
">
</script>
<script>
    ...
    createOrder: function(data, actions) {
      ...
      return actions.order.create({
        purchase_units: [{
		  ...
		  description: "Compra de artículos en <?= $N_empresa ?> por <?= CURRENCY.$total ?> ",
        }]
      });
    },
		...
		let base_url = "<?= base_url(); ?>";
			...
			$.ajax({
				...
				success: function(data) {
					if(data.status) {
						MostrarAlertaAlert("Atención", data.msg , "success");
...
<!-- <script>
	...
	<!-- breadcrumb -->
<!-- Modal -->
<div class="modal fade" id="modalTermino" tabindex="-1" aria-hidden="true">
							...
							<div class="divmetodpago">
								<div>
									<label for="paypal">
										...
										<img src="<?php echo base_url(); ?>assets/Template/Admin/images/Paypal.png" alt="Icono de PayPal" class="ml-space-sm" width="80" height="50">
									</label>
								</div>
								<div>
